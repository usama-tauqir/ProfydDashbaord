...
export default function StaffApprovalsPage() {
  ...
  useEffect(() => {
    ...
    setAuthorized(user.role === "team_lead")
  }, [user])
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .eq("request_status", "pending_teamlead")
    ...
    try {
      if (approve) {
        ...
        await supabase.from("users").upsert({
          ...
          department: req.department, role: "staff", phone: req.phone, status: "active",
        }, { onConflict: "id" })
  ...
  if (authorized === false) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 text-center">
        ...
        <p className="text-muted-foreground">Only Team Leads can approve staff account requests.</p>
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Approve or reject pending staff requests for your department ({user?.department}).
          </p>
        </div>
      ...
      {requests.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">All staff requests have been reviewed.</p>
          </CardContent>
        </Card>
      ) : (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-1">
                          ...
                          <Badge variant="secondary" className="text-xs">Staff · {req.department}</Badge>
                        </div>
                      </div>
                    </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{actionType === "approve" ? "Approve Staff Account" : "Reject Staff Account"}</DialogTitle>
            <DialogDescription>
              {actionType === "approve"
